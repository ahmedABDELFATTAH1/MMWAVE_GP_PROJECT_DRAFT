...
( function () {
		...
		this.pan = function ( deltaX, deltaY, screenWidth, screenHeight ) {
			...
			if ( scope.object instanceof THREE.PerspectiveCamera ) {
				...
				//scope.panUp( 2 * deltaY * targetDistance / screenHeight );
                                ...
                                scope.panLeft( deltaX * 1000 / window.FFTSize) ;
				scope.panUp( deltaY * 1000 / window.FFTSize );
	...
	THREE.OrbitControls = function ( object, domElement ) {
		...
		function pan( deltaX, deltaY ) {
                        deltaX = deltaX * window.FFTSize/1000;
                        deltaY = deltaY * window.FFTSize/1000;
                                ...
                                scene1.children.forEach(function(item, index)
                                    {
                                        ...
                                        if (item.name == 'AxisLabel')
                                        {
                                            
                                            if (camera1.position.z > 0)
                                            {
                                                item.rotation.y = camera1.rotation.y;
                                                item.rotation.x = camera1.rotation.x;
                                            }
                                            else
                                            {
                                                item.rotation.y = camera1.rotation.y - Math.PI;
                                                item.rotation.x = camera1.rotation.x - Math.PI;
                                            }
